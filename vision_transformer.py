...
class Mlp(nn.Module):
    ...
    def flops(self, token_len):
        if self.finetune:
            mlp_num = torch.sum(self.mlp_zeta > self.threshold).item()
            flops = token_len * (2 * self.in_features * mlp_num)
            return flops
...
class Attention(nn.Module):
    ...
    def flops(self, token_len):
        flops = 0
        if self.finetune:
            head_num = torch.sum(self.head_zeta > self.threshold).item()

            flops += token_len * self.embed_dim * 3 * self.embed_dim
            flops += 2 * head_num * token_len * self.head_dim * token_len
            flops += token_len * (head_num * self.head_dim * head_num * self.head_dim)
            return flops
...
class Block(nn.Module):
    ...
    def flops(self, token_len):
        flops = 0
        # attn
        flops += self.attn.flops(token_len)
        # mlp
        flops += self.mlp.flops(token_len)
        return flops
...
class PatchEmbed(nn.Module):
    ...
    def flops(self):
        ...
        flops = self.num_patches * self.embed_dim * 3 * (self.patch_size[0] * self.patch_size[1])
        return flops
    def forward(self, x):
        B, C, H, W = x.shape
        x = self.proj(x)
...
class VisionTransformer(nn.Module):
    ...
    def flops(self):
        flops = 0
        token_len = self.num_tokens + self.num_patches
        flops += self.patch_embed.flops()
        for i, block in enumerate(self.blocks):
            flops += block.flops(token_len)
        # head
